...
<script type="text/javascript">
    ...
    function ok${id}Clinic(tree){
        ...
        for(var i=0; i<nodes.length; i++) {//<c:if test="${checked && notAllowSelectParent}">
            ...
            if (nodes[i].isParent){
                toastr.error("不能选择父节点（"+nodes[i].name+"）请重新选择。");
                return false;
            }//</c:if><c:if test="${not empty module && selectScopeModule}">
            if (nodes[i].module == ""){
                toastr.error("不能选择公共模型（"+nodes[i].name+"）请重新选择。");
                return false;
            }else if (nodes[i].module != "${module}"){
                toastr.error("不能选择当前栏目以外的栏目模型，请重新选择。");
                return false;
            }//</c:if>
            ids.push(nodes[i].id);
       ...
       /* $("#${id}Id").val(ids.join(",").replace(/u_/ig,""));
        ...
        $.ajax({
            type:'post',
            url:'${ctx}/sys/orgGroupVsDept/save?groupId=${value}&deptIds='+ids,
