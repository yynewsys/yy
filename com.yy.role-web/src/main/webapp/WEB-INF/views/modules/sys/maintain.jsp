...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $.get("${ctx}${url}", function(zNodes){
            ...
            var menus="<c:forEach items="${menus}" var="menu">${menu},</c:forEach>";
            var ids2=menus.split(",");
    ...
    //保存系统服务分配菜单
    function saveMaintain(serviceId){
        ...
        $.ajax({
              type:'post',
              url:'${ctx}/sys/sysMenuDict/saveMenuVSService?serviceId='+serviceId+'&menusId='+ids,
    ...
    //保存自定义服务菜单
    function saveSelfMaintain(orgSelfServiceId){
        var ids = [], nodes = tree.getCheckedNodes(true);
        for(var i=0; i<nodes.length; i++) {
            ids.push(nodes[i].id);
        }
        $.ajax({
            type:'post',
            url:'${ctx}/sys/orgSelfService/saveSelfServiceMenu?selfServiceId='+orgSelfServiceId+'&menuIds='+ids,
            success:function(data){
                if(data.code=="success"){
                    parent.hrefDiv(data.data);
                }
