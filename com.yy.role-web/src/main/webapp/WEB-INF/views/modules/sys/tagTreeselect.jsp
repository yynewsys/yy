...
<script type="text/javascript">
    ...
    var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
        async:{enable:(type==3),url:"${ctx}/sys/user/treeData?timestamp="+new Date().getTime(),autoParam:["id=officeId"]},
            ...
            onClick:function(event, treeId, treeNode){
                if(${selected}){
                    if(!treeNode.isParent){
                        ...
                        parent.toastr.error("不能选择根节点（"+treeNode.name+"）请重新选择。");
                        return false;
                    }
                }else{
                    tree.expandNode(treeNode);
